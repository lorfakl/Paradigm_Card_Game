...
public class Player2 : MonoBehaviour, ICharacter
{
    // PUBLIC FIELDS
    ...
    // a script with references to all the visual game objects for this Player2
    ...
    // REFERENCES TO LOGICAL STUFF THAT BELONGS TO THIS Player2
    public Deck2 Deck2;
    public Hand hand;
    public Table table;

    // a static array that will store both players, should always have 2 players
    public static Player2[] Players;
    ...
    // total mana crystals that this Player2 has this turn
    ...
    void Awake()
    {
        // find all scripts of type Player2 and store them in Players array
        // (we should have only 2 players in the scene)
        Players = GameObject.FindObjectsOfType<Player2>();
    ...
    // draw a single card from the Deck2
    public void DrawACard(bool fast = false)
    {
        if (Deck2.cards.Count > 0)
        {
            if (hand.CardsInHand.Count < PArea.handVisual.slots.Children.Length)
            {
                // 1) logic: add card to hand
                CardLogic newCard = new CardLogic(Deck2.cards[0]);
                newCard.owner = this;
                hand.CardsInHand.Insert(0, newCard);
                // Debug.Log(hand.CardsInHand.Count);
                // 2) logic: remove the card from the Deck2
        ...
        {
            // there are no cards in the Deck2, take fatigue damage.
        }
       
    }

    // get card NOT from Deck2 (a token or a coin)
    public void GetACardNotFromDeck(CardAsset cardAsset)
    {
        if (hand.CardsInHand.Count < PArea.handVisual.slots.Children.Length)
        {
            ...
            newCard.owner = this;
            hand.CardsInHand.Insert(0, newCard);
            // 2) send message to the visual Deck2
            new DrawACardCommand(hand.CardsInHand[0], this, fast: true, fromDeck: false).AddToQueue(); 
        }
        // no removal from Deck2 because the card was not in the Deck2
    }
